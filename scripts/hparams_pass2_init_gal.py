...
tts_schedule = [(2,  1e-3, 10_000 * _tmp,  tts_batch_size, [0., 0., 1.]),   # progressive training schedule
                (2,  1e-3, 20_000 * _tmp,  tts_batch_size, [0., 0., 1.]),   # (r, lr, step, batch_size, tts_pass2_input_prob_lst: x_y_both)
                (2,  5e-4, 30_000 * _tmp,  tts_batch_size, [0., 0., 1.]),
                (2,  5e-4, 40_000 * _tmp,  tts_batch_size, [0., 0., 1.]),
...
tts_init_weights_path_pass2 = tts_init_weights_path

# tts_guided_attn_loss_lambda = 100.0
# exp_id = f'mp_lj_pass2_fixBestP1_GAL{tts_guided_attn_loss_lambda}_BS{tts_batch_size*tts_batch_acu}_stepD{tts_batch_acu}_max80k_p1fr_frL{tts_fr_length_ratio}_re4_{tts_pass2_input_train}'

# tts_guided_attn_loss_lambda = 500.0
# exp_id = f'mp_lj_pass2_fixBestP1_GAL{tts_guided_attn_loss_lambda}_BS{tts_batch_size*tts_batch_acu}_stepD{tts_batch_acu}_max80k_p1fr_frL{tts_fr_length_ratio}_re4_{tts_pass2_input_train}'
